...
#[test]
fn shell_permission_is_blocking() {
    ...
    assert!(report
        .violations
        .iter()
        .any(|v| v.id == "TS1001" && v.severity == Severity::Critical));
}

#[test]
fn filesystem_permission_is_blocking() {
    let mut manifest = base_manifest();
...
#[test]
fn global_wildcard_is_blocking() {
    let mut manifest = base_manifest();
    manifest.allowlist.domains = vec!["*".to_string()];
    let report = Enforcer::evaluate(&manifest);
    assert!(!report.passed);
    assert!(report.violations.iter().any(|v| v.id == "TS2002"));
}
...
fn strict_profile_requires_strict_csp() {
    ...
    let report = Enforcer::evaluate(&manifest);
    assert!(!report.passed);
    assert!(report.violations.iter().any(|v| v.id == "TS4001"));
}
